// Reusable "Location & Neighbourhood" POI explorer used on both the
// listing-detail page (/mls/:id) and the condo-detail page (/condos/:slug).
// Renders a CARTO-light Leaflet map with the property pin + a 1km circle,
// fetches schools/restaurants/parks/transit from the configured `poisUrl`,
// lets the visitor click any POI to see walking + driving routes back to
// the property, and lists nearby places in a sortable side panel.
//
// Lazy-loaded (React.lazy) on its host pages so react-leaflet stays out of
// the SSR bundle.
...
// Auto-fits the map to a route's bounding box when one is selected; falls
// back to centering on the property when nothing's selected.
function FitToRoute({
  coords,
  fallback,
}: {
...
export interface NeighbourhoodPoisProps {
  /** Backend path for the POI payload (e.g. `/api/mls/123/pois` or
   *  `/api/public/condos/the-river/pois`). */
  poisUrl: string;
  /** Property center coordinates (used for the marker, 1km circle, and
   *  routing origin). */
  lat: number;
  lng: number;
}

export default function NeighbourhoodPois({
  poisUrl,
  lat,
  lng,
  ...
  const { data, isLoading } = useQuery<PoisPayload>({
    queryKey: ["pois", poisUrl],
    queryFn: async () => {
      const r = await fetch(apiUrl(poisUrl));
